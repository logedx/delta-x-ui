...
export function is_exist<T, K = undefined | null>(v: T): v is Exclude<T, K> {
	return is_null(v) === false && is_undefined(v) === false

}

export function is_empty<T, K = undefined | null | ''>(v: T): v is Extract<T, K> {
...
export function is_boolean_string(v: unknown): v is string {
	return is_string(v) && (v.toLowerCase() === 'false' || v.toLowerCase() === 'true')
...
export function is_media_uri_string(v: unknown): v is string {
	return is_required_string(v) && (/^https?:\/\/[0-9a-z-_]+(\.[0-9a-z-_]+)*(:\d+)?(\/[0-9a-z-_]+)*(\/[0-9a-z-_]+\.[a-zA-Z]+)/).test(v)
...
export function is_phone_number_string(v: unknown): v is string {
	return is_required_string(v) && (/^1(3\d|4[5-9]|5[0-35-9]|6[2567]|7[0-8]|8\d|9[0-35-9])\d{8}$/).test(v)
