...
export function read_svg(src: string, color = 'none'): string {
	...
	if (
		detective.is_array_buffer(file)
	) {
		return ''
	...
	if (
		detective.is_empty_string(color)
...
type TValue<T> = T extends object ? never : T
...
export class Storage<T> {
	...
	async get(key?: unknown): Promise<unknown> {
		...
		if (
			detective.is_undefined(this.#value)
		...
		if (
			...
			if (
				detective.is_natural_number(key)

			) {
				if (
					key >= 0 && key < this.#value.length
		...
		if (
			...
			if (
				detective.is_object_keyof(key, this.#value)
	...
	async set<V extends TValue<T>>(value: V): Promise<void>

	async set<K extends TKey<T>, V extends TIvalue<T, K>>(key: K, value: V): Promise<void>
	...
	async del<K extends TKey<T>>(key: K): Promise<void> {
		if (
			detective.is_array(this.#value)
	...
	async insert<V extends TAvalue<T>>(value: V): Promise<void>

	async insert<K extends TOKey<T>, V extends TOvalue<T, K>>(key: K, value: V): Promise<void>
