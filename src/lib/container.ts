...
export type PaginationCallHandler<T, P extends object> = (params: PaginationParams<P>) => Promise<T[]>
...
> extends Array<T> {
	...
	params<K extends keyof P, V extends P[K]>(key: K, value?: V): this

	params<K extends keyof P, V extends P[K]>(
		key: 'limit' | 'sort' | K,
		value?: number | PaginationParams['sort'] | V,

	): this {
		if (key === 'limit') {
			if (
				detective.is_natural_number(value)
		...
		if (key === 'sort') {
			...
			if (
				...
				detective.is_array(value) && value.every(this.#is_sort)
...
export class Exclusive<T = object> {
